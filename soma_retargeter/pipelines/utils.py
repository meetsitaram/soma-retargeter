...
_TARGET_TYPE_TO_STR = {
    ...
    TargetType.AGIBOT_X2_ULTRA : "agibot_x2_ultra",
}
...
def get_retargeter_config(source: SourceType, target: TargetType) -> dict:
    ...
    elif target == TargetType.AGIBOT_X2_ULTRA:
        config_dir = 'agibot_x2_ultra'
        if source == SourceType.SOMA:
            filename = 'soma_to_x2_ultra_retargeter_config.json'
        else:
            raise ValueError(f"Unknown source type [{source}] for target [{target}].")
    else:
